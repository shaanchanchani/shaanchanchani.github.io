...
export default function Home() {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 overflow-x-hidden overflow-y-auto">
      <div className="max-w-3xl mx-auto space-y-6 sm:space-y-8 touch-pan-y">
        {/* Profile Section */}
        <div className="flex flex-col items-center space-y-3 sm:space-y-4">
          ...
          <div className="text-center space-y-0.5 sm:space-y-1">
            <h1 className="text-base sm:text-lg font-medium text-[#00151e]">Shaan Chanchani</h1>
            <p className="text-xs sm:text-sm text-[#00151e]/70">Purdue ECE | MLE @ Huddlevision</p>
            <p className="text-xs sm:text-sm text-[#00151e]/70">(Graduating May 2025)</p>
          </div>
          
          {/* Social Links */}
          <div className="flex space-x-4 touch-none">
            ...
            <div className="max-w-[90vw] mx-auto">
              <SwipeableCardStack>
                {articles.map((article, index) => (
                  <div 
                    key={index} 
                    className="group relative bg-white dark:bg-[#1e1e1e] rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden select-none w-[80vw] md:w-[600px] border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 p-4"
                  >
                    <div className="flex items-center gap-4">
                      <div className="relative w-24 h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-900">
                        <Image
                          src={article.image}
                          alt={article.title}
                          fill
                      ...
                      <div className="space-y-2">
                        <h3 className="text-lg font-medium text-[#00151e] dark:text-gray-100">{article.title}</h3>
                        <p className="text-sm text-[#00151e]/70 dark:text-gray-400">{article.description}</p>
                        <div className="flex flex-col space-y-1">
                          <p className="text-xs text-[#00151e]/50 dark:text-gray-500">{article.date}</p>
                          <Link 
                            href={article.url} 
            ...
            <div className="max-w-[90vw] mx-auto">
              <SwipeableCardStack>
                {projects.map((project, index) => (
                  <div 
                    key={index} 
                    className="group relative bg-white dark:bg-[#1e1e1e] rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden select-none w-[80vw] md:w-[600px] border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 p-4 space-y-4"
                  >
                    <div className="text-left">
                      <h3 className="text-lg font-medium text-[#00151e] dark:text-gray-100">{project.title}</h3>
                      <p className="text-sm text-[#00151e]/70 dark:text-gray-400 mt-1">{project.description}</p>
                    </div>
                    
                    <div className="h-[250px] bg-gray-50 dark:bg-gray-900 rounded-lg overflow-hidden">
                      {project.preview.type === 'iframe' ? (
                        <div className="relative w-[500px] h-[250px] mx-auto">
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-2">
                        {project.technologies.map((tech, techIndex) => (
                          <span key={techIndex} className="px-3 py-1 text-xs rounded-full bg-gray-100 dark:bg-gray-800 text-[#00151e] dark:text-gray-300">
                            {tech}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-col space-y-1">
                        <p className="text-xs text-[#00151e]/50 dark:text-gray-500">{project.date}</p>
                        {project.title === 'Next Gen Stats Play Visualizer' ? (
                          ...
                          <span className="text-[#00151e]/70 dark:text-gray-400 text-sm">
                            More soon...
                          </span>
                        )}
                      </div>
